	...
	*/
	...
	if (readTestCases(testFileName, testCases)){
		...
		for (int i = 0; i < testCases.size(); i++){
			...
			vector<int> resPair = sumIndicies(tc.expectedNumbers, tc.sum);
			pair<int, int> convertedPair;
			convertedPair.first = resPair[0];
			convertedPair.second = resPair[1];
			bool func2Test = (convertedPair == tc.expectedPair);
